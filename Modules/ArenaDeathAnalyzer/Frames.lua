...
---@class Arenalogs.ArenaDeathAnalyzer
Arenalogs.ARENA_DEATH_ANALYZER = Arenalogs.ARENA_DEATH_ANALYZER

---@class Arenalogs.ArenaDeathAnalyzer.Frames
Arenalogs.ARENA_DEATH_ANALYZER.FRAMES = {}

local debug = false

function Arenalogs.ARENA_DEATH_ANALYZER.FRAMES:Init()
    local sizeX = 320
    local sizeY = 445
    Arenalogs.ARENA_DEATH_ANALYZER.frame = GGUI.Frame {
        parent = UIParent, anchorParent = UIParent,
        sizeX = sizeX, sizeY = sizeY,
        frameConfigTable = ArenalogsGGUIConfig, frameID = Arenalogs.CONST.FRAMES.ARENA_DEATH_ANALYZER,
        frameTable = Arenalogs.MAIN.FRAMES,
        backdropOptions = Arenalogs.CONST.ARENA_DEATH_ANALYZER_BACKDROP,
        moveable = true, hide = true,
    }

    local content = Arenalogs.ARENA_DEATH_ANALYZER.frame.content
    ...
    content.closeButton = GGUI.Button {
        ...
        clickCallback = function()
            Arenalogs.ARENA_DEATH_ANALYZER.frame:Hide()
        end
    }
...
function Arenalogs.ARENA_DEATH_ANALYZER.FRAMES:UpdateDisplay()
    local content = Arenalogs.ARENA_DEATH_ANALYZER.frame.content

    Arenalogs.ARENA_DEATH_ANALYZER:UpdateArenaSpecIDs()
    ...
    local specIDsPlayerTeam = Arenalogs.ARENA_DEATH_ANALYZER.specIDs.PLAYER_TEAM
    local specIDsEnemyTeam = Arenalogs.ARENA_DEATH_ANALYZER.specIDs.ENEMY_TEAM
